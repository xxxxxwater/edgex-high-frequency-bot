...
更新日志 (v3.7):
- 扩展到7个币种（新增DOGE-USD, XRP-USD, HYPE-USD）
- 优化网格层数为3层（降低API压力）
- 刷新间隔调整为120秒（适配7币种，方案B）
- API间隔1.08秒（峰值0.926次/秒，留7.4%安全边际）
- 网格间距0.045%黄金配置（价差>手续费）
- 预期日交易量160万美金，年化收益1058%
...
class GridStrategyConfig:
    ...
    def __init__(self, optimization_mode: str = "ema"):
        ...
        # ===== 网格参数 =====
        self.grid_levels = 3              # 网格层数（每侧）v3.7：3层适配7币种
        
        # 根据优化模式设置网格间距
        if optimization_mode == "wider_grid":
            self.grid_spacing_pct = Decimal("0.0008")  # 0.08% - 更宽间距减少交易
        else:
            self.grid_spacing_pct = Decimal("0.00045")  # 0.045% - 超优化黄金间距（价差>手续费）
        
        self.min_grid_spacing_pct = Decimal("0.0003")  # 最小间距 0.03%
        self.max_grid_spacing_pct = Decimal("0.001")   # 最大间距 0.1%
        
        # ===== 仓位管理 =====
        self.position_size_pct = Decimal("0.06")  # 每格仓位占净值6%（超优化：避免风控）
        ...
        self.ema_position_size_pct = Decimal("0.32")  # EMA信号仓位占净值32%（超优化）
        self.ema_take_profit_pct = Decimal("0.009")   # 止盈 0.9%（超优化提高收益）
        self.ema_stop_loss_pct = Decimal("0.0045")    # 止损 0.45%（保持盈亏比2:1）
        self.ema_min_signal_interval = 1000  # 最小信号间隔16.7分钟（超优化提高质量）
        ...
        # ===== 订单管理 =====
        self.order_refresh_interval = 120  # 订单刷新间隔（秒）v3.7：120秒适配7币种（方案B）
        self.min_order_interval = 1.6     # 最小下单间隔（秒）
        self.max_open_orders = 60         # 最大挂单数量（3层×2侧×7币种=42，留余量）
        self.api_call_interval = 1.08     # API调用间隔（秒）(0.926次/秒，留7.4%安全边际)
        ...
        self.MIN_ORDER_SIZES = {
            ...
            "DOGE-USD": Decimal("10"),      # v3.7新增
            "XRP-USD": Decimal("5"),        # v3.7新增
            "HYPE-USD": Decimal("1"),       # v3.7新增（根据价格调整）
        }
